...
void input_manager_reset_all_inputs() {
	...
	// If there is at least one keyboard using the default keymap and repeat
	// delay and rate, then it is possible that there is a keyboard group that
	// needs to be disarmed. Since all inputs are being reset anyway, just
	// disarm all of them.
	struct sway_seat *seat;
	wl_list_for_each(seat, &server.input->seats, link) {
